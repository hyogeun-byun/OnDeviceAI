...
- 🤸 **실시간 자세 동기화** — MoveNet SinglePose Lightning(TFLite **int8**)로 COCO 17 keypoint를 추론, 8개 관절 각도 유사도로 텔레파시 게이지 계산
- 🧠 **온디바이스 LLM 진행자** — EXAONE 3.5 2.4B (Ollama)가 한국어로 실시간 코칭 멘트 생성ㅖ
...
[카메라 보드 N대]  ──WebSocket(JPEG + 키포인트)──▶  [서버 보드]  ──WebSocket──▶  [브라우저]
  ...
  · MoveNet int8 (5fps)                              · 게임 상태머신 (10Hz)          · /stage 관객
...
| 용도 | 모델·엔진 | 위치 | 비고 |
|------|-----------|------|------|
| 자세(포즈) 추정 | **MoveNet SinglePose Lightning** (TFLite **int8**, 192×192) | 카메라 워커 | COCO 17 keypoint · CPU 추론 p50 15ms/p95 17ms(4스레드, XNNPACK) · 게임은 5fps로 스로틀 |
| 동작 유사도 점수 | **규칙 기반** — 8개 관절 각도 코사인 유사도 × 활동성 게이트 | 서버 | 픽셀 좌표 미사용, 카메라 위치·거리에 강건 |
...
| 포즈 추정 | **MoveNet SinglePose Lightning TFLite int8** (tflite-runtime) |
...
워커가 MoveNet으로 뽑은 **키포인트(COCO 17점)만** 서버로 전송합니다(영상은 표시용 JPEG 별도). 서버는 각 플레이어의 8개 관절(양쪽 팔꿈치·어깨·고관절·무릎) 각도 벡터를 구해 플레이어 간 코사인 유사도 평균을 냅니다. 여기에 **활동성 게이트**를 곱해 "모두 가만히 서 있으면 유사도가 높아도 낮은 점수"가 되도록 설계해 꼼수를 방지합니다.
