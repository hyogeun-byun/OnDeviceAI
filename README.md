...
[카메라 워커 N대] -- WebSocket(JPEG + pose keypoints) --> [FastAPI 서버] -- WebSocket --> [브라우저]
 camera-worker                                             server/app        /, /game, /stage
...
- `camera-worker/app/inference/pose_estimator.py`: MediaPipe Pose 실행 및 33 landmarks를 keypoint 배열로 변환
- `camera-worker/app/constants.py`: `KEYPOINT_INFERENCE_FPS = 10.0`
- `server/app/services/pose_similarity.py`: 얼굴 방향, 상체 bone vector, 활동성 게이트 계산
- `server/app/services/stream_manager.py`: 카메라별 `keypoint_count` 집계
...
Python 의존성은 `server/requirements.txt`, `camera-worker/requirements.txt`에 `==` 버전 고정 형식으로 관리한다.
...
| `test-results/requirements/R-01-R-27-requirements-unittest.log` | 제출/평가용 unittest 원본 |
...
상세 절차는 `test-results/offline-lan/R-26-offline-lan-guide.md`에 있다.
