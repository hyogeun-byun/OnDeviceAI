...
- 🤸 **실시간 자세 동기화** — MediaPipe Pose로 관절 33개를 추론, 8개 관절 각도 유사도로 텔레파시 게이지 계산
...
[카메라 보드 N대]  ──WebSocket(JPEG + 키포인트)──▶  [서버 보드]  ──WebSocket──▶  [브라우저]
  ...
  · MediaPipe Pose (5fps)                            · 게임 상태머신 (10Hz)          · /stage 관객
...
| 용도 | 모델·엔진 | 위치 | 비고 |
|------|-----------|------|------|
| 자세(포즈) 추정 | **MediaPipe Pose** (`model_complexity=0`) | 카메라 워커 | 33개 랜드마크, 5fps 추론 |
...
| 포즈 추정 | **MediaPipe Pose** (mediapipe ≥ 0.10.14) |
...
워커가 MediaPipe로 뽑은 **키포인트만** 서버로 전송합니다(영상은 표시용 JPEG 별도). 서버는 각 플레이어의 8개 관절(양쪽 팔꿈치·어깨·고관절·무릎) 각도 벡터를 구해 플레이어 간 코사인 유사도 평균을 냅니다. 여기에 **활동성 게이트**를 곱해 "모두 가만히 서 있으면 유사도가 높아도 낮은 점수"가 되도록 설계해 꼼수를 방지합니다.
