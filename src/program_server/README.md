...
- 시작 화면에서 **카테고리** 1개를 고른다: `상황 / 운동 / 감정 / 영화 혹은 애니메이션 / 인물(for 2,30대) / 인물(for 4,50대)`
...
- 점수는 MediaPipe Pose landmark에서 만든 **얼굴 방향 + 상체 6개 bone vector 유사도**에 **활동성 게이트**를 곱한다. 다 같이 **가만히 서 있으면**(중립 자세) 유사도가 높아도 점수가 낮게 깔려서, 가만히 있는 “꼼수”를 막고 과감한 동작을 유도한다.
...
[카메라 보드 N개]  --WebSocket(JPEG+키포인트)-->  [서버 보드]  --WebSocket-->  [브라우저/프로젝터]
  ...
  · MediaPipe Pose Lite                           · 게임 상태머신(10Hz)          · mp3 음성 재생
  · 10fps 키포인트 추론                            · LLM/TTS 백그라운드           · 결과 PNG 저장
...
| 용도 | 모델 / 엔진 | 위치 | 비고 |
|------|-------------|------|------|
| 자세(포즈) 추정 | **MediaPipe Pose Lite** (`model_complexity=0`, 입력 폭 192px) | 카메라 워커 | 33 landmarks, 라즈베리파이 평균 약 30ms · 게임은 10fps로 스로틀(영상 전송과 스레드 분리) |
| 동작 유사도 점수 | 규칙 기반(모델 아님) — **얼굴 방향 + 상체 6개 bone vector 코사인 유사도 × 활동성 게이트** | 서버 | `app/services/pose_similarity.py`, 카메라 거리/위치에 강건 · 가만히 있으면 저점 |
...
1. **포즈 → 점수**: 워커가 MediaPipe Pose Lite로 뽑은 33개 landmark를 서버로 보낸다(영상은 표시용 JPEG 별도). 서버는 얼굴 방향 2개와 양팔 상체 4개를 합친 6개 bone vector를 만들고, 플레이어들 간 방향 유사도를 평균해 0~100 "텔레파시 게이지"로 환산한다. 픽셀 좌표가 아니라 **정규화된 방향 벡터**를 쓰므로 카메라 위치·거리·키 차이에 강하다.
...
pip install openai            # src/program_server/.venv 안에서
...
```bash
pip install edge-tts          # src/program_server/.venv 안에서 (requirements.txt에 포함)
```
...
- 노트북 브라우저에서 `http://<server-ip>:8000/game` 접속 → 팀 이름 입력 후 **T자 포즈**로 시작(첫 상호작용이 오디오 재생 권한을 풀어줌) → 헤드폰으로 조용히 청취. 우상단 🔊/🔇로 음소거.

### 게임 진행 흐름 (시작 트리거 → 인트로 → 라운드 → 리포트)

1. **idle**: 카테고리 선택 + 팀 이름 입력 후, 모든 플레이어가 **양팔 T자 포즈**를 약 1.5초 유지하면 게임이 자동 시작된다(시작 버튼 없음).
2. **intro**: AI MC 민수(2D 애니메이션 아바타)의 오프닝 멘트가 끝나면 곧바로 `3·2·1` 카운트다운 후 라운드가 시작된다.
...
성능 시각화는 기본으로 켜져 있다. 끄려면 `src/program_server/.env`에 다음 값을 넣는다.
