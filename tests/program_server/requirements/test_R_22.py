...
class TestR22SpeechIdDedup(unittest.TestCase):
    def test_speech_id_prevents_duplicate_voice_playback_and_falls_back(self):
        ...
        assert_contains(self, "src/program_server/app/api/game_routes.py", '@router.get("/speech/{speech_id}.mp3")')
        assert_contains(
            self,
        ...
        write_requirement_log(
            ...
            "expected_runtime_logs=browser console log or screen recording plus test-results/program_server/requirements/R-22-speech-id-dedup-fallback-server.log",
        )
