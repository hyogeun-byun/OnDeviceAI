...
class TestR26OfflineLanOperation(unittest.TestCase):
    def test_core_game_is_local_with_optional_network_tts_fallback(self):
        assert_contains(self, "src/program_server/.env.example", "LLM_ENABLED=false", "EDGE_TTS_ENABLED=true")
        assert_contains(self, "src/program_camera_worker/.env.example", "SERVER_URL=http://192.168.0.10:8000")
        assert_contains(
            self,
        ...
        write_requirement_log(
            ...
            "field_success_criteria=WAN disconnected, server, camera worker, and browser on same LAN, /game and /stage finish one 60-second sprint",
            "expected_runtime_logs=test-results/program_camera_worker/requirements/R-26-offline-lan-server.log",
            "expected_camera_logs=test-results/program_camera_worker/requirements/R-26-offline-lan-camera-<CAMERA_NO>.log",
            "guide=test-results/program_camera_worker/offline-lan/R-26-offline-lan-guide.md",
        )
