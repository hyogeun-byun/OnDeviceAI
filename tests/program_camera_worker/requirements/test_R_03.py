...
class TestR03MediaPipePose(unittest.TestCase):
    def test_mediapipe_lite_33_landmarks_10fps_is_declared_and_used(self):
        assert_contains(
            self,
            "src/program_camera_worker/app/inference/pose_estimator.py",
            "MediaPipe Pose",
            "Outputs 33 landmarks",
            '_BACKEND = "mediapipe"',
            "model_complexity=model_complexity",
        )
        assert_contains(self, "src/program_camera_worker/app/constants.py", "KEYPOINT_INFERENCE_FPS = 10.0")
        assert_contains(self, "src/program_camera_worker/requirements.txt", "mediapipe==0.10.18")
        assert_contains(
            self,
            "src/program_camera_worker/app/metrics.py",
            "frame_fps",
            "pose_fps",
        ...
        assert_contains(
            self,
            "docs/requirements/SW_Bootcamp_13기_A반_3팀_요구사항_명세서.md",
            "MediaPipe Pose Lite",
            "33개",
            "9fps 이상",
        )
    ...
    def test_worker_metrics_writes_actual_fps_jsonl(self):
        metrics_module = load_path_module("camera_worker_metrics_for_test", "src/program_camera_worker/app/metrics.py")
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            write_requirement_log(
                ...
                "expected_runtime_logs=test-results/program_camera_worker/requirements/R-03-pose-fps-33-landmarks-camera-<CAMERA_NO>.log",
                "expected_metric_logs=test-results/program_camera_worker/requirements/R-03-pose-fps-33-landmarks-camera-<CAMERA_NO>-metrics.jsonl",
